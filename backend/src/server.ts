...
app.post('/api/dashboard', async (req: Request, res: Response) => {
    ...
    var verified=false
    if (jwtToken) {
        jwt.verify(jwtToken, jwtSecret, async (err: any) => {
            if (err) {
                return res.status(403).send({message:"Invalid token.", error:err}); // Unauthorized access
...
app.patch('/api/update', async (req: Request, res: Response) => {
    ...
    // Validate payload
    if (!updatedData) {
        res.status(400).json({ message: "Invalid request: Updated Patient data is empty" });
    }
    else if ( typeof updatedData !== 'object' || !updatedData.id || !updatedData.active || !updatedData.name) {
        res.status(400).json({ message: "Invalid request: Patient data missing required fields" });
    }
    try {
        ...
        if (verified){
            ...
            res.status(201).send({message: 'Your changes have been saved successfully.'})
            }
